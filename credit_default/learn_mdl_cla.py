...
def make_model(target_enc_feats=[], es_groups=[]):
    ...
    if len(es_groups):
        df_lrn['credit_problems'] = df_lrn.NumberOfCreditProblems.astype(
            'category')
        dict_purposes = {p: i for i, p in
                         enumerate(df_lrn.grp_purpose.unique())}
        df_lrn['num_purpose'] = df_lrn.grp_purpose.map(dict_purposes)
        col_stratify = 'num_purpose'
    else:
        col_stratify = 'grp_purpose'
    ...
    group_cols = ['HomeOwnership', 'grp_purpose', 'debt_group',
                  'rest_bal_group']
    if len(es_groups):
        group_cols.append('credit_problems')
    ...
    # print(train_lrn[mdl_columns].info())
    ...
    # max_depth      6.000000
    # train_score    0.682928
    # valid_score    0.505557
    #    max_depth  train_score  valid_score
    # 4          6     0.682928     0.505557
    # 1          3     0.569575     0.504156
    # 3          5     0.644202     0.504016
    # 5          7     0.732535     0.503908
    # 2          4     0.605193     0.503480
...
if __name__ == "__main__":
    ...
    enc_feats = [
        ...
        'grp_purpose_cla_mean',
        'grp_purpose_cla_med_mean',
        'debt_group_cla_median',
        'debt_group_cla_mean',
        'debt_group_cla_med_mean',
        'rest_bal_group_cla_median',
        'rest_bal_group_cla_mean',
        'rest_bal_group_cla_med_mean',
        'NumberOfCreditProblems_cla_median',
        'NumberOfCreditProblems_cla_mean',
        'NumberOfCreditProblems_cla_med_mean',
        'HomeOwnership_cla_median',
        'HomeOwnership_cla_mean',
        'HomeOwnership_cla_med_mean'
    ]
    # for target_feat in enc_feats:
    #     # target_feats = ['NumberOfCreditProblems_cla_median', target_feat]
    #     target_feats = [target_feat]
    #     make_model(target_enc_feats=target_feats)
    # 'NumberOfCreditProblems_cla_median', 'grp_purpose_cla_median']
    # make_model(target_enc_feats=['NumberOfCreditProblems_cla_median'])
    # best_params={'depth': 3, 'iterations': 270}
    make_model(target_enc_feats=['NumberOfCreditProblems_cla_median',
                                 'grp_purpose_cla_median'])
    # best_params={'depth': 3, 'iterations': 320}
