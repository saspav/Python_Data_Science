...
def make_model(target_enc_feats=[], es_groups=[]):
    ...
    if len(es_groups):
        ...
        with open(os.path.join(PATH_EXPORT, 'model_cols.csv'), 'w') as mc:
            for col in mdl_columns:
                mc.write(f'{col}\n')
        # это не нужно, т.к. уже все поделено
        learn_cats = []
    ...
    # max_depth      4.000000
    # train_score    0.517689
    # valid_score    0.357565
...
if __name__ == "__main__":
    ...
    enc_feats = [
        # 'grp_purpose_cs_median',
        # 'grp_purpose_cs_mean',
        # 'grp_purpose_cs_med_mean',
        'debt_group_cs_median',
        'debt_group_cs_mean',
        'debt_group_cs_med_mean',
        'rest_bal_group_cs_median',
        'rest_bal_group_cs_mean',
        'rest_bal_group_cs_med_mean',
        'NumberOfCreditProblems_cs_median',
        'NumberOfCreditProblems_cs_mean',
        'NumberOfCreditProblems_cs_med_mean',
        'HomeOwnership_cs_median',
        'HomeOwnership_cs_mean',
        'HomeOwnership_cs_med_mean'
    ]
    # debt_group_cs_mean - второй по рейтингу
    # for target_feat in enc_feats:
    #     target_feats = ['grp_purpose_cs_median', target_feat]
    #     # target_feats = [target_feat]
    #     make_model(target_enc_feats=target_feats)
    # make_model(target_enc_feats=['grp_purpose_cs_median'])
    # best_params={'depth': 4, 'iterations': 750}
    # make_model(target_enc_feats=['debt_group_cs_mean'])
    # best_params = {'depth': 4, 'iterations': 760}
    # make_model(target_enc_feats=['grp_purpose_cs_median',
    #                              'rest_bal_group_cs_med_mean'])
    # best_params={'depth': 4, 'iterations': 700}
